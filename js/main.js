...
// --- DOM 元素 (unchanged + 新增 Best Sellers) ---
...
let activeBestsellerSiteFilter = 'all'; // 新增
...
// --- Utility: escape HTML to avoid injecting raw markup into innerHTML ---
...
// Resolve a URL for a given bestseller record and a column type
function resolveBestsellerUrl(item, col) {
    // col: 'category1' | 'category1Zh' | 'subcategory' | 'subcategoryZh'
    const lookup = {
    ...
    // fallback to generic item.url
    if (item.url) return item.url;
    // fallback: construct Amazon search URL using site and the column text
...
function startRealtimeListeners() {
    ...
    if (unsubscribers.sellers || unsubscribers.keywords || unsubscribers.products || unsubscribers.bestsellers) return;
    ...
    const debouncedProcess = () => {
        ...
        snapshotDebounceTimer = setTimeout(() => {
            processStoreData(); processKeywordData(); processProductData(); processBestsellerData();
            hideLoading();
...
function renderSiteFilters(container, dataArray, type) {
    ...
    const activeFilter = type === 'store' ? activeStoreSiteFilter
...
bestsellerDateHeader.addEventListener('click', () => {
    bestsellerSortDir = bestsellerSortDir === 'desc' ? 'asc' : 'desc'; bestsellerSortKey = 'createdAt';
    updateSortIcon(bestsellerDateHeader, bestsellerSortDir); renderBestsellerTable();
});
...
function renderStoreRow(item, index) {
    ...
    // Build seller name cell with link if possible:
...
function renderBestsellerRow(item, index) {
    ...
    // resolve urls for each column, prefer explicit URL fields, fallback to site search URL
...
function renderBestsellerTable() {
    ...
    if (rows.length === 0) {
        ...
        bestsellerSelectAll.disabled = true; bestsellerSelectAll.checked = false;
        bestsellerVirtualizer.updateData([], renderBestsellerRow);
        return;
    } else bestsellerSelectAll.disabled = false;
...
async function deleteBestseller(id) {
    ...
    try { await db.collection('amazonBestsellers').doc(id).delete(); showNotification('Best Sellers 数据删除成功！', 'success'); selectedBestsellerIds.delete(id); }
    catch (err) { console.error("删除失败: ", err); showNotification('删除失败，请检查控制台。', 'error'); }
}
...
bestsellerSelectAll?.addEventListener('change', (e) => {
    const isChecked = e.target.checked; const current = getFilteredSortedBestsellers().map(i => i.id);
    if (isChecked) current.forEach(id => selectedBestsellerIds.add(id)); else current.forEach(id => selectedBestsellerIds.delete(id));
...
storesTab.addEventListener('click', () => {
    ...
    storesTab.classList.add('active'); keywordsTab.classList.remove('active'); productsTab.classList.remove('active'); bestsellersTab.classList.remove('active');
    storesView.classList.remove('hidden'); keywordsView.classList.add('hidden'); productsView.classList.add('hidden'); bestsellersView.classList.add('hidden');
...
keywordsTab.addEventListener('click', () => {
    ...
    keywordsTab.classList.add('active'); storesTab.classList.remove('active'); productsTab.classList.remove('active'); bestsellersTab.classList.remove('active');
    keywordsView.classList.remove('hidden'); storesView.classList.add('hidden'); productsView.classList.add('hidden'); bestsellersView.classList.add('hidden');
    processKeywordData(); updateSortIcon(keywordDateHeader, keywordSortDir);
});
productsTab.addEventListener('click', () => {
    if (productsTab.classList.contains('active')) return;
    productsTab.classList.add('active'); storesTab.classList.remove('active'); keywordsTab.classList.remove('active'); bestsellersTab.classList.remove('active');
    productsView.classList.remove('hidden'); storesView.classList.add('hidden'); keywordsView.classList.add('hidden'); bestsellersView.classList.add('hidden');
...
bestsellersTab?.addEventListener('click', () => {
    ...
    bestsellersTab.classList.add('active'); storesTab.classList.remove('active'); keywordsTab.classList.remove('active'); productsTab.classList.remove('active');
    bestsellersView.classList.remove('hidden'); storesView.classList.add('hidden'); keywordsView.classList.add('hidden'); productsView.classList.add('hidden');
    processBestsellerData(); updateSortIcon(bestsellerDateHeader, bestsellerSortDir);
});
...
function processBestsellerData() { renderSiteFilters(bestsellerFiltersDiv, Array.from(bestsellerData.values()), 'bestseller'); renderBestsellerTable(); }
